-- ── Migración 033: fn_welcome_context con códigos de artefacto ───────────────
-- Añade kr_code / obj_code / code a los tres bloques de la función de bienvenida.
-- Requiere aplicar con superusuario postgres (okr_user no tiene CREATE FUNCTION).

CREATE OR REPLACE FUNCTION fn_welcome_context(
  p_org_id  UUID,
  p_user_id UUID
)
RETURNS JSON LANGUAGE plpgsql STABLE AS $$
DECLARE
  v_result JSON;
BEGIN
  SELECT json_build_object(

    -- Datos del usuario
    'user', (
      SELECT json_build_object(
        'id',   u.id,
        'name', u.name,
        'role', u.role
      )
      FROM users u WHERE u.id = p_user_id
    ),

    -- Ciclo activo principal (preferencia: QUARTERLY > ANNUAL)
    'active_cycle', (
      SELECT json_build_object(
        'id',             c.id,
        'name',           c.name,
        'type',           c.type,
        'start_date',     c.start_date,
        'end_date',       c.end_date,
        'days_remaining', GREATEST(0, (c.end_date - CURRENT_DATE)),
        'days_elapsed',   GREATEST(0, (CURRENT_DATE - c.start_date)),
        'total_days',     (c.end_date - c.start_date),
        'cycle_pct',      ROUND(
          100.0 * GREATEST(0, (CURRENT_DATE - c.start_date)::NUMERIC)
          / NULLIF((c.end_date - c.start_date)::NUMERIC, 0)
        )
      )
      FROM cycles c
      WHERE c.organization_id = p_org_id
        AND c.status = 'ACTIVE'
        AND c.deleted_at IS NULL
      ORDER BY
        CASE c.type WHEN 'QUARTERLY' THEN 1 WHEN 'ANNUAL' THEN 2 ELSE 3 END
      LIMIT 1
    ),

    -- Mis OKRs activos (+ código de artefacto)
    'my_objectives', (
      SELECT COALESCE(json_agg(json_build_object(
        'id',       o.id,
        'code',     o.code,
        'title',    o.title,
        'status',   o.status,
        'level',    o.level,
        'kr_count', (SELECT COUNT(*) FROM key_results kr WHERE kr.objective_id = o.id AND kr.deleted_at IS NULL)
      ) ORDER BY o.created_at), '[]'::JSON)
      FROM objectives o
      WHERE o.organization_id = p_org_id
        AND o.owner_id = p_user_id
        AND o.deleted_at IS NULL
        AND o.status NOT IN ('CANCELLED', 'COMPLETED')
        AND EXISTS (
          SELECT 1 FROM cycles c
          WHERE c.id = o.cycle_id AND c.status = 'ACTIVE'
        )
    ),

    -- KRs pendientes de check-in (+7 días sin actualizar) (+ códigos)
    'pending_checkins', (
      SELECT COALESCE(json_agg(json_build_object(
        'kr_id',           kr.id,
        'kr_code',         kr.code,
        'kr_title',        kr.title,
        'obj_code',        o.code,
        'objective_title', o.title,
        'days_since',      COALESCE(
          EXTRACT(DAY FROM (NOW() - kr.last_checkin_at))::INT,
          999
        ),
        'last_checkin_at', kr.last_checkin_at,
        'confidence',      kr.confidence,
        'progress',        kr.progress
      ) ORDER BY kr.last_checkin_at ASC NULLS FIRST), '[]'::JSON)
      FROM key_results kr
      JOIN objectives o ON kr.objective_id = o.id
      WHERE o.organization_id = p_org_id
        AND kr.owner_id = p_user_id
        AND kr.deleted_at IS NULL
        AND o.deleted_at IS NULL
        AND kr.status NOT IN ('CANCELLED', 'COMPLETED')
        AND EXISTS (
          SELECT 1 FROM cycles c WHERE c.id = o.cycle_id AND c.status = 'ACTIVE'
        )
        AND (
          kr.last_checkin_at IS NULL
          OR kr.last_checkin_at < NOW() - INTERVAL '7 days'
        )
      LIMIT 5
    ),

    -- KRs en riesgo de la organización (+ códigos)
    'at_risk_krs', (
      SELECT COALESCE(json_agg(json_build_object(
        'kr_id',           kr.id,
        'kr_code',         kr.code,
        'kr_title',        kr.title,
        'obj_code',        o.code,
        'objective_title', o.title,
        'confidence',      kr.confidence,
        'progress',        kr.progress,
        'level',           o.level
      ) ORDER BY kr.confidence ASC), '[]'::JSON)
      FROM key_results kr
      JOIN objectives o ON kr.objective_id = o.id
      WHERE o.organization_id = p_org_id
        AND kr.deleted_at IS NULL
        AND o.deleted_at IS NULL
        AND kr.confidence < 0.4
        AND kr.status NOT IN ('CANCELLED', 'COMPLETED')
        AND EXISTS (
          SELECT 1 FROM cycles c WHERE c.id = o.cycle_id AND c.status = 'ACTIVE'
        )
      LIMIT 4
    ),

    -- Próximos eventos de gobernanza (próximos 14 días)
    'upcoming_governance', (
      SELECT COALESCE(json_agg(json_build_object(
        'event_type',     g.event_type,
        'title',          g.title,
        'responsible',    g.responsible,
        'deliverable',    g.deliverable,
        'scheduled_date', g.scheduled_date,
        'due_date',       g.due_date,
        'status',         g.status
      )), '[]'::JSON)
      FROM fn_governance_calendar(p_org_id, 'ANNUAL') g
      WHERE g.status IN ('UPCOMING', 'IN_PROGRESS', 'OVERDUE')
        AND g.scheduled_date <= CURRENT_DATE + INTERVAL '14 days'
        AND g.scheduled_date >= CURRENT_DATE - INTERVAL '7 days'
      LIMIT 4
    ),

    -- Estadísticas globales de la organización
    'org_stats', (
      SELECT json_build_object(
        'total_objectives',  COUNT(*),
        'on_track',          COUNT(*) FILTER (WHERE o.status = 'ON_TRACK'),
        'at_risk',           COUNT(*) FILTER (WHERE o.status = 'AT_RISK'),
        'completed',         COUNT(*) FILTER (WHERE o.status = 'COMPLETED'),
        'at_risk_krs', (
          SELECT COUNT(*)
          FROM key_results kr2
          JOIN objectives o2 ON kr2.objective_id = o2.id
          WHERE o2.organization_id = p_org_id
            AND kr2.deleted_at IS NULL AND o2.deleted_at IS NULL
            AND kr2.confidence < 0.4
            AND kr2.status NOT IN ('CANCELLED', 'COMPLETED')
            AND EXISTS (
              SELECT 1 FROM cycles c WHERE c.id = o2.cycle_id AND c.status = 'ACTIVE'
            )
        )
      )
      FROM objectives o
      WHERE o.organization_id = p_org_id
        AND o.deleted_at IS NULL
        AND EXISTS (
          SELECT 1 FROM cycles c WHERE c.id = o.cycle_id AND c.status = 'ACTIVE'
        )
    )

  ) INTO v_result;

  RETURN v_result;
END;
$$;
